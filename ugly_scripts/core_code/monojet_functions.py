...
import math
import monox_functions

## Believe that full cross section is not really analytically integrable.
## New attempt is to do it numerically per point.
ECM = 13000.**2

# Imported from cython
# These handle all the actual PDFs and low level calculations
# making it way faster than pure python.
integrand_parton_vector = sc.LowLevelCallable.from_cython(monox_functions,
                                               'integrand_parton_vector')

integrand_hadronic_vector = sc.LowLevelCallable.from_cython(monox_functions,
                                               'integrand_hadronic_vector')

integrand_parton_axialvector = sc.LowLevelCallable.from_cython(monox_functions,
                                               'integrand_parton_axialvector')

integrand_hadronic_axialvector = sc.LowLevelCallable.from_cython(monox_functions,
                                               'integrand_hadronic_axialvector')                               
...
def relative_monox_xsec_parton_vector(M, mDM, gq, gDM, gl) :
  ...
  integral = integrate.quad(integrand_parton_vector,4.*mDM**2,ECM,args=(gamma,M,mDM),points=intpoints,limit=500)
...
def relative_monox_xsec_parton_axial(M, mDM, gq, gDM, gl) :
  ...
  # it discover the resonance peak.
  intpoints = [M,M**2-gamma,M**2,M**2+gamma]
  integral = integrate.quad(integrand_parton_axialvector,4.*mDM**2,ECM,args=(gamma,M,mDM),points=intpoints,limit=500)
  # Use when checking integral precision
  #print("Est.",integral[0],"pm",integral[1])
  xsec = gq**2 * gDM**2 * integral[0]
  return xsec
...
def relative_monox_xsec_hadron_vector(M, mDM, gq, gDM, gl) :
  ...
  for q_pid in range(1,3) :
    ...
    integral = integrate.nquad(integrand_hadronic_vector,[limit_x1,limit_x2],args=(q_pid,gamma,M,mDM),opts=[opts_x1,opts_x2])    
...
def relative_monox_xsec_hadron_axial(M, mDM, gq, gDM, gl) :
  ...
  for q_pid in range(1,3) :
    ...
    # Limits and options are functions.
    # The limits are similarly functions.    
    integral = integrate.nquad(integrand_hadronic_axialvector,[limit_x1,limit_x2],args=(q_pid,gamma,M,mDM),opts=[opts_x1,opts_x2],full_output=True) 
    xsec = xsec + integral[0]
  return gq**2 * gDM**2 * xsec
